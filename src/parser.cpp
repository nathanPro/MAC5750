...
AST::Exp Parser::Exp()
{
    ...
    case Lexeme::open_paren:
        builder << Lexeme::open_paren << Exp() << Lexeme::close_paren;
...
AST::Stm Parser::Stm()
{
    ...
    case Lexeme::if_keyword:
        builder << Lexeme::if_keyword << Lexeme::open_paren << Exp()
                << Lexeme::close_paren << Stm()
    ...
    case Lexeme::println_keyword:
        builder << Lexeme::println_keyword << Lexeme::open_paren
                << Exp() << Lexeme::close_paren << Lexeme::semicolon;
...
AST::MethodDecl Parser::MethodDecl()
{
    ...
    while (Lexeme(tokens[0]) != Lexeme::return_keyword &&
        ...
        builder << Stm();
    builder << Lexeme::return_keyword << Exp() << Lexeme::semicolon
            << Lexeme::close_brace;
